...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col lg:flex-row bg-[#05050a] text-white font-sans overflow-hidden selection:bg-cyan-500/30 relative">
      
      {/* Background */}
      <div className="absolute inset-0 z-0 overflow-hidden">
        <div className="absolute top-0 -left-4 w-96 h-96 bg-indigo-600 rounded-full mix-blend-multiply filter blur-[128px] opacity-40 animate-pulse"></div>
        <div className="absolute top-0 -right-4 w-96 h-96 bg-cyan-600 rounded-full mix-blend-multiply filter blur-[128px] opacity-40"></div>
        <div className="absolute -bottom-32 left-20 w-96 h-96 bg-fuchsia-600 rounded-full mix-blend-multiply filter blur-[128px] opacity-40"></div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-8 lg:px-20 py-12 relative z-10">
        <div className="flex items-center gap-3 mb-16">
          <div className="relative group">
            ...
            <div className="relative bg-slate-900 p-2.5 rounded-lg border border-white/10">
          ...
          <span className="text-sm font-bold tracking-widest uppercase text-slate-400">CryptoChart<span className="text-cyan-400">.AI</span></span>
        </div>

        <div className="max-w-2xl space-y-6 mb-16">
          <h1 className="text-5xl lg:text-6xl font-extrabold tracking-tight leading-[1.1]">
            See The<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-indigo-400 to-fuchsia-400">Matrix.</span>
          </h1>
          <p className="text-lg text-slate-400 font-light leading-relaxed max-w-lg">
            Upload any crypto chart. Our AI decodes liquidity, structure, and sentiment in seconds. Four powerful analysis modes at your fingertips.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 max-w-lg">
          <div className="bg-white/5 border border-white/10 p-5 rounded-2xl backdrop-blur-sm hover:bg-white/10 transition-colors group">
            <span className="text-2xl mb-3 block group-hover:scale-110 transition-transform duration-500">📈</span>
            <h3 className="text-sm font-semibold text-white mb-1">Vision Mode</h3>
            <p className="text-[11px] text-slate-500 leading-normal">Pattern recognition & price action analysis.</p>
          </div>
          <div className="bg-white/5 border border-white/10 p-5 rounded-2xl backdrop-blur-sm hover:bg-white/10 transition-colors group">
            <span className="text-2xl mb-3 block group-hover:scale-110 transition-transform duration-500">⚡</span>
            <h3 className="text-sm font-semibold text-white mb-1">Signal Mode</h3>
            <p className="text-[11px] text-slate-500 leading-normal">Probability metrics & alpha decoding.</p>
          </div>
          <div className="col-span-2 bg-gradient-to-r from-cyan-500/10 to-indigo-500/10 border border-cyan-500/20 p-5 rounded-2xl flex items-center justify-between group cursor-default">
            <div>
              <h3 className="text-sm font-semibold text-white mb-1">AI-Powered Analysis</h3>
              <p className="text-[11px] text-slate-400">Verify wins, scan market pulse, decode charts.</p>
            </div>
            <div className="w-10 h-10 rounded-full bg-cyan-500/20 flex items-center justify-center group-hover:rotate-12 transition-transform duration-500">
      ...
      <div className="flex-1 relative flex items-center justify-center p-8 lg:p-0 z-20 overflow-hidden">
        <div className="relative w-full max-w-[380px] bg-slate-900/90 backdrop-blur-2xl border border-white/10 p-8 rounded-3xl shadow-2xl shadow-cyan-500/5">
          <div className="mb-6 text-center">
            <h2 className="text-2xl font-bold text-white mb-2">{isLogin ? 'Welcome Back' : 'Get Started'}</h2>
            <p className="text-xs text-slate-500 uppercase tracking-widest font-medium">
              {isLogin ? 'Access your dashboard' : 'Start analyzing charts today'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            {!isLogin && (
              <div className="relative">
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 pl-10 pr-4 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-cyan-500 focus:bg-white/10 transition-all"
            ...
            <div className="relative">
              ...
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 pl-10 pr-11 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-cyan-500 focus:bg-white/10 transition-all"
            ...
            <button type="submit" disabled={isLoading}
              className="w-full py-3.5 bg-gradient-to-r from-cyan-500 to-indigo-500 text-white rounded-xl font-bold text-sm hover:from-cyan-400 hover:to-indigo-400 transition-all shadow-lg shadow-cyan-500/25 active:scale-[0.98] flex items-center justify-center gap-2 mt-1">
              {isLoading ? (
                <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin"></div>
              ) : (
                <>
          ...
          <div className="mt-5 text-center">
            <button onClick={() => { setIsLogin(!isLogin); setError(''); setResetSent(false); setPassword(''); }}
              className="text-xs text-slate-500 hover:text-cyan-400 transition-colors border-b border-transparent hover:border-cyan-400 pb-0.5">
